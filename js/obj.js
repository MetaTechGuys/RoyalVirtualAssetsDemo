...
class Object3DVisualizer {
  ...
  async init() {
    ...
    setTimeout(() => {
      const zoomInstruction = document.querySelector(
        ".object-3d-zoom-instruction"
      );
      if (zoomInstruction) {
    ...
    if (container) {
      ...
      if (this.touchHandlers) {
        container.removeEventListener(
          "touchstart",
          this.touchHandlers.touchstart
        );
        container.removeEventListener(
          "touchmove",
          this.touchHandlers.touchmove
        );
  ...
  createGradientBackground() {
    ...
    // Create diagonal gradient from blue to white to blue (more blue presence)
    // 45-degree angle gradient for elegant dynamic effect
    
    // GRADIENT ANGLE OPTIONS:
    // 0° (horizontal):     (0, 0, canvas.width, 0)
    // 45° (diagonal):      (0, canvas.height, canvas.width, 0) 
    // 90° (vertical):      (0, 0, 0, canvas.height)
    // 135° (reverse diag): (0, 0, canvas.width, canvas.height)
    // 30° (custom):        (0, canvas.height, canvas.width, canvas.height * 0.4)
    // 60° (custom):        (0, canvas.height, canvas.width * 0.6, 0)
    
    const gradient = context.createLinearGradient(0, 0, 0, canvas.height)
    gradient.addColorStop(0, "#16213e"); // More prominent blue left
    gradient.addColorStop(0.8, "#e3f2fd"); // Light blue
    gradient.addColorStop(1, "#ffffff"); // White center
    // gradient.addColorStop(0.8, "#e3f2fd"); // Light blue
    // gradient.addColorStop(1, "#1e88e5"); // More prominent blue right
  ...
  setupScene() {
    ...
    // Create horizontal gradient background (blue-white-blue)
    this.createGradientBackground();

    // Add fog for depth (reduced density for white background)
    this.scene.fog = new THREE.FogExp2(this.config.backgroundColor, 0.0002);
    ...
    // No shadow or lighting calculations needed for white background

    container.appendChild(this.renderer.domElement);

    // No lights needed for white background - better performance and no washing out
  ...
  createCentralObject(centralObj) {
    ...
    // Create a bright material for white background (optimized for image display)
    const material = new THREE.MeshBasicMaterial({
      map: null,
      transparent: false,
      color: new THREE.Color(0xffffff), // White base color for proper image display
      opacity: 1.0,
    });

    // Create mesh
    const mesh = new THREE.Mesh(geometry, material);
    mesh.castShadow = false; // Disabled for white background
    ...
    // No glow effects for better performance
  ...
  addPulsingAnimation(mesh) {
    if (window.gsap) {
      ...
      // No color pulsing to preserve image brightness and colors

      // No glow meshes - better performance
  ...
  createOrbitingObjects(orbitingObjects) {
    ...
    orbitingObjects.forEach((customObj, index) => {
      ...
      const objectsInThisOrbit = orbitingObjects.filter(
        (_, i) => Math.floor(i / 3) === Math.floor(index / 3)
      );
      ...
      // Define image file path
      const imagePath = `assets/object-images/${customObj.name
        .toLowerCase()
        .replace(/\s+/g, "-")}.webp`;
  ...
  addMultipleOrbitPaths() {
    ...
    for (let i = 1; i <= maxOrbits; i++) {
      ...
      const material = new THREE.LineBasicMaterial({
        color: new THREE.Color().setHSL(0.6, 0.9, 0.3), // Darker for white background
        // color: new THREE.Color().setHSL(0.6 + i * 0.1, 0.8, 0.6),
        transparent: true,
        opacity: 0.6 - i * 0.06, // More visible opacity
      ...
      if (window.gsap) {
        gsap.to(material, {
          opacity: (0.7 - i * 0.06) * 1.6, // Adjusted for white background
          duration: 2 + i * 0.4,
          repeat: -1,
          yoyo: true,
          ease: "sine.inOut",
  ...
  createFallbackTexture(customObj, material, index) {
    ...
    // Draw text (no shadows for better performance)
  ...
  getColorForObject(name, index, isCentral = false) {
    // Special colors for central object (lighter blue for white background)
    if (isCentral) {
      return "#64b5f6";
    }

    // Predefined colors for different categories (lighter blues for white background)
    ...
    // Generate lighter color based on index if no category match
    const hue = (index * 137.508) % 360;
    return `hsl(${hue}, 70%, 60%)`;
  }

  addPostProcessingEffects() {
    // No lighting effects needed for white background
    // Objects rely on emissive materials for visibility
  }
  ...
  addFloatingAnimation(mesh) {
    if (window.gsap) {
      ...
      // No color manipulation to preserve image brightness
    ...
    if (window.gsap) {
      ...
      gsap.to(mesh.userData, {
        ...
        onUpdate: () => {
          ...
          if (
            mesh.userData.isOrbiting &&
            !mesh.userData.isSelected &&
            mesh !== this.hoveredObject
          ) {
  ...
  async showObjectInfo(customObj) {
    ...
    const isCentral = this.objectModels.find(
      (obj) => obj.userData.customObject === customObj && obj.userData.isCentral
    );
    ...
    if (isMobile) {
      ...
      <div class="object-3d-info-actions">
        <button class="action-button details-button" data-object="${
          customObj.id || customObj.name
        }">View Details</button>
        ${
          customObj.actionUrl
            ? `<button class="action-button action-url-button" data-url="${customObj.actionUrl}">Open</button>`
            : ""
        }
      </div>
    `;
    ...
    if (detailsButton) {
      const handleDetails = () => {
        ...
        const fullDescription =
          customObj.description || "No additional details available.";
    ...
    this.objectModels.forEach((obj) => {
      ...
      if (obj.userData.isOrbiting) {
        ...
        if (obj.userData.isSelected) {
          ...
          obj.position.y =
            obj.userData.selectedPosition.y +
            floatingOffset +
        ...
        if (obj !== this.hoveredObject) {
          ...
          if (pathPoints && pathPoints.length > 0) {
            const normalizedAngle =
              (obj.userData.orbitAngle % (Math.PI * 2)) / (Math.PI * 2);
            const pointIndex = Math.floor(
              normalizedAngle * (pathPoints.length - 1)
            );
            ...
            obj.position.z = newZ;
            obj.position.y =
              obj.userData.orbitHeight + (obj.userData.floatingOffset || 0);
          }
  ...
  update() {
    if (this.initialized) {
      ...
      if (this.selectedObject && this.selectedObject.userData.customObject) {
        selectedId =
          this.selectedObject.userData.customObject.id ||
          this.selectedObject.userData.customObject.name;
      }
      ...
      if (selectedId) {
        const newObject = this.objectModels.find(
          (obj) =>
            obj.userData.customObject &&
            (obj.userData.customObject.id === selectedId ||
              obj.userData.customObject.name === selectedId)
        );
...
class CustomObjectAPI {
  ...
  removeObject(identifier) {
    const index = this.objects.findIndex(
      (obj) => obj.id === identifier || obj.name === identifier
    );
    ...
    const sampleObjects = [
      {
        ...
        description:
          "Royal Virtual Assets is a new global atmosphere for investment and trading, RVA stands for transparency, efficiency, and security in blockchain technology.",
        isCentral: true,
      },
      {
        name: "Tokenisation",
        description:
          "A data security process that involves substituting a sensitive data element with a non-sensitive equivalent, known as a token.",
      },
      {
        name: "Wallet",
        description:
          "A type of financial transaction app that runs on any connected device. It securely stores your payment information and passwords.",
      },
      {
        name: "Trade Platform",
        description:
          "A cryptocurrency trading platform is an online portal (exchange) that facilitates crypto-to-crypto transactions.",
      },
      {
        name: "Coin",
        description:
          "Cryptocurrencies are digital assets that rely on an encrypted network to execute, verify, and record transactions.",
      },
      {
        name: "Dex",
        description:
          "Decentralized exchanges are platforms that crypto traders can connect with using a web3 crypto wallet in order to perform trades.",
      },
      {
        name: "Swap",
      ...
      },
      {
        name: "IDO",
        description:
          "An initial DEX offering (IDO) is a fundraising method for blockchain-based projects through decentralized exchanges (DEXs).",
      },
      {
        name: "ICO",
        description:
